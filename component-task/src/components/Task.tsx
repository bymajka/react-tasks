...
const Task = (props: {id: symbol, title: string; description: string; isCompleted: boolean; iterationValue: number; categorie: string, onToggle: (id: symbol) => void}) => {
    return(
        <div className={`flex flex-row items-center gap-2 bg-purple-400 min-w-xl rounded-md justify-between m-4 p-4 border-0 border-b-4 border-l-2 border-r-2 ${props.isCompleted ? 'border-green-500' : 'border-red-700'}`}>
            ...
            <TaskCategoryInput id={props.id} categorie={props.categorie} />
